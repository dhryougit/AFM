...
class FrequencyLoss(nn.Module):
    ...
    def forward(self, pred, target):
        ...
        self.coef = self.coef.to(pred.device)
        preds_fq = (preds_fq * self.coef).sum(dim=1).unsqueeze(dim=1) + 16.
        target_fq = (target_fq * self.coef).sum(dim=1).unsqueeze(dim=1) + 16.

        preds_fq, target_fq = preds_fq / 255., target_fq / 255.
           


        return self.loss_weight * self.scale * torch.log(((preds_fq - target_fq) ** 2).mean(dim=(1, 2, 3)) + 1e-8).mean()
